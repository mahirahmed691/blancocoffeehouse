...
function safeReceiptUrl(url) {
  try {
    ...
    if (!/(^|\.)stripe\.com$/i.test(parsed.hostname)) return "";
    return parsed.href;
  } catch (err) {
    return "";
  }
...
async function receiptUrlForSession(sessionId) {
  var stripe = stripeClient();
  ...
  try {
    ...
    var charge = intent.latest_charge;
...
module.exports = {
  ...
  receiptUrlForSession: receiptUrlForSession,
  safeReceiptUrl: safeReceiptUrl,
