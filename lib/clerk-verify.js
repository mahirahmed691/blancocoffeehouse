...
function allowedParty(azp) {
  ...
  if (
    ...
    origin === "com.blancocoffeehouse.app" ||
    origin === "blanco"
  ) {
    return true;
  }
  ...
  if (origin.indexOf("exp://") === 0) return true;
  if (origin.indexOf("blanco://") === 0) return true;
  if (/^https:\/\/blancocoffeehouse-[a-z0-9-]+\.vercel\.app$/.test(origin)) return true;
  if (/\.clerk\.accounts\.dev$/.test(origin.replace(/^https:\/\//, ""))) return true;
  return false;
}
